...
- [x] **PROMPT-02**: each major prompt documented — what it does, why it is structured that way, and how it handles unreliable / missing / conflicting information.
...
- [x] **PROMPT-04**: ≥1 documented prompt failure example + the fix, plus prompt versioning/evaluation notes (§21 differentiator).

### Data Generation (20%)

- [x] **DATA-01**: generate one realistic marketing-services RFQ via prompt — 8 line items, scope, timelines, item requests, commercial expectations, vendor questionnaire, compliance — feels like a real procurement event, not a clean sample.
- [x] **DATA-02**: generate ≥3 deliberately messy vendor responses via prompt, each driven by an explicit per-vendor "mess spec" so they differ in pricing structure, completeness, scope coverage, timelines, assumptions, and clarity.
- [x] **DATA-03**: generated vendors inject real-world complexity (missing/incomplete pricing, unclear tax/currency/assumptions/exclusions, partial scope, vague timelines, weak compliance); messiness is asserted in tests so the data is never too clean to test.
- [x] **DATA-04**: the generated RFQ + ≥3 vendor responses are committed as sample data AND regenerable live in-app (dynamic, never hardcoded outputs).
...
- [x] **UI-03**: Extraction Review screen — per-vendor extracted fields with highlighted important fields, missing/unclear/conflicting data, risks, and visible evidence snippets.
- [x] **UI-04**: Vendor Comparison screen — side-by-side across the 6 dimensions; shows who is comparable, where they differ, and what needs further review.
- [x] **UI-05**: Prompt Trace / Prompt Pack screen — the prompts used plus ≥1 full trace.
...
- [x] **SHIP-01**: web deployed to Vercel and AI service deployed to Render/Railway, wired via env-configured base URL (CORS + disabled proxy buffering so SSE streams).
- [x] **SHIP-02**: README — setup, run instructions, model/API requirements, env vars, sample flow, assumptions.
- [x] **SHIP-03**: 1–2 page write-up — problem, assumptions, prompt architecture, product thinking, UI/UX decisions, extraction approach, comparison approach, limitations, what's next.
- [x] **SHIP-04**: ≤5-min demo video covering system flow, how prompts are used, data generation, extraction, comparison, and messy/exception-case handling.
- [x] **SHIP-05**: architecture diagram (system + AI pipeline) in `docs/` (§21 differentiator).
...
*Last updated: 2026-06-29 — traceability synced to phase verifications during v1.0 milestone audit (all 31 v1 reqs verified satisfied)*
